...
def parse_whatsapp_chat(uploaded_file, chat_format):
    ...
    for line in lines:
        if chat_format == 'Android':
            match = re.match(r'(\d{1,2}/\d{1,2}/\d{2}, \d{1,2}:\d{2}\s[AP]M) - ', line)
        else:  # iPhone
            match = re.match(r'\[(\d{1,2}/\d{1,2}/\d{2}, \d{1,2}:\d{2}):.*\s[AP]M\] ', line)

        if match:
            if current_message['sender'] or current_message['message']:
                parsed_data.append({
            ...
            sender_message = line[len(match.group(0)):].strip()

            if ': ' in sender_message:
                sender, message = sender_message.split(': ', 1)
            else:
                sender, message = None, sender_message
...
def load_data():
    ...
    chat_format = st.radio("Select chat format", ("iPhone", "Android"))

    if uploaded_file is not None and chat_format:
        return parse_whatsapp_chat(uploaded_file, chat_format)
    else:
        return None
...
def perform_eda(whatsapp_df):
    # Attempt to convert timestamp strings to datetime objects without seconds
    timestamp_format = '%m/%d/%y, %I:%M %p'  # Common format for both iPhone and Android
    whatsapp_df['timestamp'] = pd.to_datetime(whatsapp_df['timestamp'], format=timestamp_format, errors='coerce')

    # Drop rows where timestamp could not be parsed
    whatsapp_df = whatsapp_df.dropna(subset=['timestamp'])

    # Further processing after successful parsing
    ...
    if not messages_per_sender.empty:
        plt.subplot(4, 1, 2)
    ...
    if not messages_per_day_of_week.empty:
        plt.subplot(4, 1, 3)
    ...
    if not messages_per_hour.empty:
        plt.subplot(4, 1, 4)
...
def perform_date(whatsapp_df):
    # Common timestamp format for both iPhone and Android (without seconds)
    timestamp_format = '%m/%d/%y, %I:%M %p'

    # Attempt to convert timestamp strings to datetime objects
    whatsapp_df['timestamp'] = pd.to_datetime(whatsapp_df['timestamp'], format=timestamp_format, errors='coerce')
...
def perform_sentiment_analysis(whatsapp_df):
    ...
    # Ensure that 'date' column is present and contains valid dates
    if 'date' not in whatsapp_df.columns or whatsapp_df['date'].isnull().all():
        print("Error: 'date' column is missing or contains no valid dates.")
        return pd.DataFrame()  # Return an empty DataFrame to prevent further processing
...
def visualize_sentiment_analysis(whatsapp_df):
    ...
    plt.subplot(1, 2, 1)
    if 'polarity' in whatsapp_df.columns and not whatsapp_df['polarity'].empty:
        sns.histplot(whatsapp_df['polarity'], kde=True, bins=30)
        plt.title('Distribution of Sentiment Polarity')
        plt.xlabel('Polarity Score')
        plt.ylabel('Frequency')
    else:
        plt.text(0.5, 0.5, 'No polarity data available', horizontalalignment='center', verticalalignment='center')

    # Average Sentiment Polarity per Day
    if 'date' in whatsapp_df.columns and not whatsapp_df['date'].empty:
        avg_polarity_per_day = whatsapp_df.groupby('date')['polarity'].mean()
        plt.subplot(1, 2, 2)
        if not avg_polarity_per_day.empty:
            avg_polarity_per_day.plot(kind='line', color='blue')
            plt.title('Average Sentiment Polarity per Day')
            plt.xlabel('Date')
            plt.ylabel('Average Polarity Score')
        else:
            plt.text(0.5, 0.5, 'No average polarity data available', horizontalalignment='center', verticalalignment='center')
    else:
        plt.text(0.5, 0.5, 'No date data available', horizontalalignment='center', verticalalignment='center')
...
def main():
    ...
    st.image('assets/streamlit_01.jpg', width=500)
